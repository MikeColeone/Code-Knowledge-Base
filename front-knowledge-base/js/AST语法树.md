...
AST（Abstract Syntax Tree，抽象语法树）并不是某一个具体时间点 “发明” 的单一产物，而是随着编译原理和程序语言处理技术的发展逐步形成的核心概念：

1. **技术溯源** ：其思想雏形可追溯到 20 世纪 50-60 年代，当时编译器技术开始快速发展（如 FORTRAN、COBOL 等早期编程语言的编译器开发），编译器需要将人类编写的代码转换为机器可执行的指令，而 AST 是连接 “源代码文本” 和 “机器码” 的关键中间表示。
2. **正式成型** ：到 20 世纪 70-80 年代，随着编译原理体系化（如经典的 “编译原理龙书”《Compilers: Principles, Techniques, and Tools》出版），AST 作为编译过程中 “语法分析阶段” 的核心输出被正式定义和标准化。
3. **广泛应用** ：2000 年后，随着动态语言（JavaScript、Python）、静态代码分析、前端工程化（Babel、ESLint）的兴起，AST 从编译器领域走向通用的程序分析 / 转换场景，成为前端、后端、DevOps 等领域的基础工具。

我们可以把一段程序抽象出类似json结构，进而表示出其抽象的树状结构就是抽象语法树。这两步也就是关键的词法分析和语法分析。
...
## 三、AST 的实际应用场景为了解决什么问题?

AST 不只是编译器的玩具，它在**很多开发工具和框架中都有广泛的应用，直接操作代码文本非常复杂**：
