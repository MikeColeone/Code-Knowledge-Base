...
浏览器早期是单线程模型，在IE6的时候，页面渲染，js执行，插件运行在一个进程里

在不存在进程隔离的时代，页面非常不稳定，直接导致几种情况：

- 浏览器中一个插件挂了，整个浏览器直接跟着挂掉
- js脚本如果比较耗时，整个页面直接卡死
- 插件可以轻松拿到浏览器内存，网页安全性很差

后期演变成多线程模型：

- 主进程
  - 很符合设计理念，我们学编程都知道一个程序只会有一个主函数，或者说只有一个入口，这个进程扮演着类似的角色：总指挥，它负责管理页面，管理子进程的创建和销毁，最高的权限，只有它可以和操作系统的文件读写，窗口显示等核心能力
- 网络进程
  - 我把它理解为中间传输的物流部门，负责资源的加载，http/https、缓存、cookie管理等之前的一段时间他只是浏览器主进程的一个模块，后来把它抽离出来，也是为了解耦和安全，防止资源加载的进程权限太高，威胁到操作系统
- GPU进程
  - 浏览器的门面，负责的是特效、美工，现代浏览器不像当年的浏览器，最初的浏览器展示的页面很简单，甚至只是为了展示黄历，后续随着页面的负责，绘制的动画交给GPU处理远远强于CPU
- 渲染进程
  - 前端最重要的部分，将前端html，css，js转化成用户可交互的网页，运行在sandbox中，无法写硬盘或者调用系统接口，它包括很多线程：
    - GUI渲染进程： 这里不同于GPU的进程，他的任务主要是处理解析HTML DOM树，解析CSS为CSSOM树，计算layout,最后进行重绘重排
    - JS引擎线程：解析js脚本 和 GUI线程是互斥的线程，这里为什么互斥的？ 其实很好理解，这似乎编程中的静态条件，举个例子，浏览器GUI线程正在渲染页面，但是这时候突然执行了tag.remove 删除标签，这时候元素就会丢失，渲染进程就会直接挂掉，因为他要渲染的东西找不到了，所以浏览器干脆解耦，分离了两个线程
    - 事件触发线程：处理事件循环机制
    - 定时器触发线程：settimeout setInterval 计时
